...
#less noise
epsLessNoise <- rnorm(100, sd = (sqrt(0.25) * 0.25))
yLessNoise <- -1 + (0.5 * x) + epsLessNoise

#more noise
epsMoreNoise <- rnorm(100, sd = 0.5)
yMoreNoise <- -1 + (0.5 * x) + epsMoreNoise
```

```{r}
fit_lm <- function(train, test) {
  LinModel <- lm(y ~ x, data = train)
  pred <- predict(LinModel, newdata = test)
  return(pred)
}
```
...
```{r}
library(boot)
cv_orig <- cv.glm(data = data.frame(x, y), glmfit = fit_lm, K = 10)
cv_LessNoise <- cv.glm(data = data.frame(x, y_LessNoise), m = fit_lm, K = 10)
cv_MoreNoise <- cv.glm(data = data.frame(x, y_MoreNoise), m = fit_lm, K = 10)
```
